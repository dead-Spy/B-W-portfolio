...
// Fix 1: MotionPathHelper is only needed for development/debugging.
// It is recommended to remove it from production build to reduce file size.
// I'm keeping the import and register here for development purposes, but 
// in a production-ready application, you should conditionally load it or remove it.
...
// Fix 2: Removed redundant `fadeIn(textId, ...)` call inside onStart.
// The `autoAlpha: 1` already handles the fade-in, preventing double animation.
const animateSplitText = (
  id: string | string[],
  _textId: string, // Kept this parameter, but it's not used internally anymore
  duration: number = 0.8,
  ...
  gsap.to(id, {
    onStart: () => {
      // fadeIn(textId, 100, 2); // Removed the redundant fadeIn call
      onStartFn();
    },
    ...
    duration: duration,
    y: 0,
    autoAlpha: 1, // This handles the fade-in of the letters/words
    stagger: stagger,
    ease: 'power4.inOut',
  });
};
...
const animateHeroNav = () => {
  ...
  // TANVIR: Fade In + Scale Up animation
  gsap.fromTo(
